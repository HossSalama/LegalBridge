    ...
    public class HearingRepository : GenericRepository<Hearing>, IHearingRepository
    {
          ...
          string? searchTerm) 
        {
            ...
            if (!string.IsNullOrEmpty(statusFilter) && statusFilter != "الكل ")
            {
                ...
                {
                    "قادم" => query.Where(h => h.AttendanceStatus == AttendanceStatusEnum.Incoming),
                    "مكتمله" => query.Where(h => h.AttendanceStatus == AttendanceStatusEnum.Attended),
                    "غائب" => query.Where(h => h.AttendanceStatus == AttendanceStatusEnum.Absent),
                    _ => query
                };
            }
