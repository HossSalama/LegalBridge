...
namespace smartLaywer.Services.ClassService
{
    public class CaseService : ICaseService
    {
        ...
        /// Returns null if not found ó caller must handle null.
        /// </summary>
        ...
        public async Task ValidateClientIdentifierUniquenessAsync(Client client)
        {
            if (client.ClientType == ClientTypeEnum.Individual)
            {
                if (string.IsNullOrWhiteSpace(client.NationalId))
                    throw new InvalidOperationException("«·—ﬁ„ «·ﬁÊ„Ì „ÿ·Ê» ··√›—«œ.");
                ...
                if (duplicate)
                    throw new InvalidOperationException($"«·—ﬁ„ «·ﬁÊ„Ì '{client.NationalId}' „”Ã· „”»ﬁ« ·⁄„Ì· ¬Œ—.");
            }
            else
            {
                if (string.IsNullOrWhiteSpace(client.CommercialReg))
                    throw new InvalidOperationException("—ﬁ„ «· ”ÃÌ· «· Ã«—Ì „ÿ·Ê» ··‘—ﬂ«  Ê«·ÃÂ«  «·ÕﬂÊ„Ì….");
                ...
                if (duplicate)
                    throw new InvalidOperationException($"—ﬁ„ «· ”ÃÌ· '{client.CommercialReg}' „”Ã· „”»ﬁ« ·⁄„Ì· ¬Œ—.");
            }
        }
        ...
        {
            // Load tracked entity ó Update() will change only the fields we set
            ...
            // so Update() just marks it Modified ó no duplicate tracking issue.
